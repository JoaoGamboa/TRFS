...
public class W74CarFollowing extends CarFollowingModel {
	...
	public float update(float dX, float dV, float length1, float Vn, float Vn1,
		...
		if (dX <= ABX) {
			...
			CX = CXconst.getCurrentVal()
					* (CXadd.getCurrentVal() + CXmult.getCurrentVal() + (RND1 + RND2));
			SDV = (float) Math.pow(((dX - length1 - AX) / CX), 2);

			if (dX < SDX) {
				OPDV = SDV
						* (-OPDVadd.getCurrentVal() - OPDVmult.getCurrentVal()
								* NRND);
		...
		case 2:
			return (float) Math.min(0.5
					* (Math.pow(dV, 2) / (ABX - dX - length1)) + an1,
					(-20 + 1.5 * Vn / 60));

		case 3:
			return dV > OPDV ? 1 : -1 * BNULLmult.getCurrentVal()
					* (RND3 + NRND);
