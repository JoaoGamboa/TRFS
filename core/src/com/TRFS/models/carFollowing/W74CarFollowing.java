...
public class W74CarFollowing extends CarFollowingModel {
	...
	@Override
	public float update(float dX, float dV, float length1, float Vn, float Vn1, float an1, float maxSpeed, float desiredSpeed) {
		
		AX = length1 + AXadd.getCurrentVal() + RND1 * AXmult.getCurrentVal();
		BX = (float) ((BXadd.getCurrentVal() + BXmult.getCurrentVal() * RND1) * Math.sqrt(dV > 0 ? Vn : Vn1));
		ABX = AX + BX;
		...
		case 4:
			FaktorV = maxSpeed/(desiredSpeed) 
						+ FAKTORVmult.getCurrentVal() * (maxSpeed-desiredSpeed);
