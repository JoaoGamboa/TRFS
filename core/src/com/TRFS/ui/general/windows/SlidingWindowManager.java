...
public class SlidingWindowManager {
	...
	/**
	 * Manages the {@link SlidingWindow}s on the screen, closing the ones that
	 * get overlapped.
	 * 
	 */
	...
	public void manage(SlidingWindow clickedWindow) {
		...
		if (!clickedWindow.isVisible()) {
			...
			for (SlidingWindow closingWindow : list) {
				if (!closingWindow.equals(clickedWindow)
						&& closingWindow.isVisible()
						&& !closingWindow.isRepositioned()
						&& closingWindow.isDockLeft() == clickedWindow
								.isDockLeft()) {
