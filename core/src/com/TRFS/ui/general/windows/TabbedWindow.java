...
public class TabbedWindow extends SlidingWindow {
	...
	public TabbedWindow (String title, float targetWidth, float targetHeight, Stage stage, boolean dockLeft, boolean dockDown, String[] buttons, ArrayList<Table> tables) {
		...
		for (int i = 0; i < buttons.length; i++) {
			...
			if (i == 1) { cell.pad(5,0,5,1);}
			else if (i == buttons.length - 1) {cell.pad(5,1,5,0).row();}
			else {cell.pad(5,0,5,0);}
		...
		super.add(new Image(skin, "horBezel")).fill().height(4).colspan(buttons.length).row();
	...
	private void changeTab(Actor clickedActor, final Cell<Table> cell) {
		final Table clickedTable = tableList.get(buttonList.indexOf(clickedActor));
				
		for (final Table table : tableList) {
			if (table.isVisible() && !table.equals(clickedTable)) {
				table.addAction(Actions.sequence(Actions.parallel(Actions.fadeOut(0.25f), Actions.moveBy(200, 0, 0.25f)),
							Actions.run(new Runnable() {
								@Override
								public void run() {
									buttonList.get(tableList.indexOf(table)).setChecked(false);
									table.setVisible(false);
	...
	private void setTab(Actor clickedActor) {
		...
		clickedTable.addAction(Actions.parallel(Actions.moveBy(-200, 0, 0.25f),Actions.fadeIn(0.25f)));
	}
}
