...
public class SlidingWindow extends Window {
	...
	/**
	 * A {@link SlidingWindow} allows the user to move it along the screen in
	 * order to be able to display multiple {@link Window}s. When dragged all
	 * the way to one side of the {@link Screen}, the window gets docked.
	 * 
	 * @param title
	 * @param targetWidth
	 * @param targetHeight
	 ...
	 * @param dockDown
	 */
	public SlidingWindow(String title, float targetWidth, float targetHeight,
			Stage stage, boolean dockLeft, boolean dockDown) {
		super(title, skin);
	...
	public void show(final SlidingWindow clickedWindow) {
		...
		this.addAction(Actions.parallel(
				Actions.sizeTo(this.targetWidth, this.targetHeight, 0.25f),
	...
	public void hide(final SlidingWindow closingWindow) {
		this.addAction(Actions.sequence(Actions.parallel(
				Actions.sizeTo(0, this.targetHeight, 0.25f),
				Actions.moveTo(this.hideX, this.startY, 0.25f)), Actions
				.run(new Runnable() {
					@Override
					public void run() {
	...
	public void resize(int width, int height) {
		...
		this.targetHeight = this.targetHeight
				+ (height - this.previousScreenHeight);
	...
	private void setHidingConditions(float width, float height) {
		if (dockDown) {
			this.startY = 0;
		} else {
			this.startY = height - TopBarTable.height - height;
		}
		if (dockLeft) {
			this.startX = 0;
			hideX = 0;
