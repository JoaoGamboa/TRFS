...
public class SimulationParamWindow {
	...
	public static void create(SlidingWindow window) {
		...
		Scenario currentScenario = ((WorldScreen) ScreensManager.getWorldScreen()).getScenario();
		
		for (int i = 0; i < SimulationParameters.simParamsListBase.length; i++) {
			
			sliderListBase.add( new DynamicParamSlider(SimulationParameters.simParamsListBase[i]));
		...
		for (Link link : currentScenario.getMap().getInFlowLinks()) {
			DynamicParamSlider slider = new DynamicParamSlider(link.getInFlowParam());
	...
	public static void resetParameters() {
		...
		for (int i = 0; i < ((WorldScreen) ScreensManager.getWorldScreen()).getScenario().getMap().getInFlowLinks().size; i++) {
			DynamicSimParam param = SimulationParameters.simParamsListFlows.get(i);
			param.setCurrentVal(param.getDefaultVal());
